(() => {
  ...
  const ARRAY_KEY = 'questions';
  const SINGULAR = 'Booking Question';
  const FIELDS = ["event_type_id", "label", "help_text", "question_type", "required", "options", "sort_order", "active"];
  ...
  function render() {
    ...
    const cards = $("cards");
    cards.innerHTML = rows.map((r) => `<article class="card"><div><span class="badge ${r.active === false || r.enabled === false ? "off" : ""}">${r.status ? escapeHtml(r.status) : r.active === false || r.enabled === false ? "INACTIVE" : "ACTIVE"}</span><h3>${escapeHtml(r.label)}</h3><div class="muted">${escapeHtml(`${r.question_type || "field"}${r.required ? " · Required" : ""} · ${(data.services.find(x => x.id === r.event_type_id) || {}).name || "All appointment types"}`)}</div></div><div class="actions"><button type="button" data-edit="${r.id}">Edit</button><button type="button" data-delete="${r.id}">Delete</button></div></article>`).join("");
