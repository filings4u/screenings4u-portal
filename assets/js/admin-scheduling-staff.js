(() => {
  ...
  let data = {};
  const ENTITY = 'staff';
  const ARRAY_KEY = 'staff';
  const SINGULAR = 'Staff Member';
  const FIELDS = ["display_name", "email", "phone", "timezone", "default_schedule_id", "active"];
  ...
  function render() {
    ...
    const cards = $("cards");
    cards.innerHTML = rows.map((r) => `<article class="card"><div><span class="badge ${r.active === false || r.enabled === false ? "off" : ""}">${r.status ? escapeHtml(r.status) : r.active === false || r.enabled === false ? "INACTIVE" : "ACTIVE"}</span><h3>${escapeHtml(r.display_name)}</h3><div class="muted">${escapeHtml(r.email || r.phone || r.timezone || "")}</div></div><div class="actions"><button type="button" data-edit="${r.id}">Edit</button><button type="button" data-delete="${r.id}">Delete</button></div></article>`).join("");
