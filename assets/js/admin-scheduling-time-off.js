(() => {
  ...
  let data = {};
  const ENTITY = 'time_off';
  const ARRAY_KEY = 'time_off';
  const SINGULAR = 'Time Off';
  const FIELDS = ["staff_id", "start_at", "end_at", "reason", "all_day"];
  ...
  function render() {
    ...
    const cards = $("cards");
    cards.innerHTML = rows.map((r) => `<article class="card"><div><span class="badge ${r.active === false || r.enabled === false ? "off" : ""}">${r.status ? escapeHtml(r.status) : r.active === false || r.enabled === false ? "INACTIVE" : "ACTIVE"}</span><h3>${escapeHtml((data.staff.find(x => x.id === r.staff_id) || {}).display_name || "Staff")}</h3><div class="muted">${escapeHtml(`${new Date(r.start_at).toLocaleString()} – ${new Date(r.end_at).toLocaleString()}${r.reason ? " · " + r.reason : ""}`)}</div></div><div class="actions"><button type="button" data-edit="${r.id}">Edit</button><button type="button" data-delete="${r.id}">Delete</button></div></article>`).join("");
