(() => {
  ...
  const ARRAY_KEY = 'overrides';
  const SINGULAR = 'Override';
  const FIELDS = ["schedule_id", "override_date", "override_type", "start_time", "end_time", "reason"];
  ...
  function render() {
    ...
    const cards = $("cards");
    cards.innerHTML = rows.map((r) => `<article class="card"><div><span class="badge ${r.active === false || r.enabled === false ? "off" : ""}">${r.status ? escapeHtml(r.status) : r.active === false || r.enabled === false ? "INACTIVE" : "ACTIVE"}</span><h3>${escapeHtml(`${r.override_date || "Date"} · ${r.override_type || "Override"}`)}</h3><div class="muted">${escapeHtml(`${(data.schedules.find(x => x.id === r.schedule_id) || {}).name || "Schedule"}${r.start_time ? " · " + r.start_time + "–" + r.end_time : ""}${r.reason ? " · " + r.reason : ""}`)}</div></div><div class="actions"><button type="button" data-edit="${r.id}">Edit</button><button type="button" data-delete="${r.id}">Delete</button></div></article>`).join("");
