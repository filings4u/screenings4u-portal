(() => {
  ...
  const ARRAY_KEY = 'waitlist';
  const SINGULAR = 'Waitlist Entry';
  const FIELDS = ["event_type_id", "requested_date", "requested_start_at", "attendee_name", "attendee_email", "attendee_phone", "status", "notes"];
  ...
  function render() {
    ...
    const cards = $("cards");
    cards.innerHTML = rows.map((r) => `<article class="card"><div><span class="badge ${r.active === false || r.enabled === false ? "off" : ""}">${r.status ? escapeHtml(r.status) : r.active === false || r.enabled === false ? "INACTIVE" : "ACTIVE"}</span><h3>${escapeHtml(r.attendee_name || r.attendee_email || "Waitlist customer")}</h3><div class="muted">${escapeHtml(`${(data.services.find(x => x.id === r.event_type_id) || {}).name || "Appointment"} · ${r.requested_date || "Any date"} · ${r.status || "waiting"}`)}</div></div><div class="actions"><button type="button" data-edit="${r.id}">Edit</button></div></article>`).join("");
