(() => {
  ...
  const FIELDS = ["name", "location_type", "address_line1", "address_line2", "city", "state", "postal_code", "country", "timezone", "phone", "instructions", "active"];
  ...
  function render() {
    ...
    const cards = $("cards");
    cards.innerHTML = rows.map((r) => `<article class="card"><div><span class="badge ${r.active === false || r.enabled === false ? "off" : ""}">${r.status ? escapeHtml(r.status) : r.active === false || r.enabled === false ? "INACTIVE" : "ACTIVE"}</span><h3>${escapeHtml(r.name)}</h3><div class="muted">${escapeHtml([r.address_line1, r.city, r.state, r.postal_code].filter(Boolean).join(", ") || r.location_type || "")}</div></div><div class="actions"><button type="button" data-edit="${r.id}">Edit</button><button type="button" data-delete="${r.id}">Delete</button></div></article>`).join("");
